...
const SLUGS: Record<string, string> = {
  "blitz-classic": "deep-shell-words",
  "blitz-survival": "deep-shell-words-blitz-survival",
  "wrapper-classic": "deep-shell-words-guided",
  "wrapper-survival": "deep-shell-words-wrapper-survival",
  "crack-classic": "deep-shell-words-crack",
  "crack-survival": "deep-shell-words-crack-survival",
};
...
const MODE_DESCRIPTIONS: Record<string, string> = {
  "blitz-classic": "Find as many deep shell words as you can in 90 seconds. Score more for longer outer words.",
  "blitz-survival": "Enter a deep shell word every 8 seconds or it's game over. Each correct word resets the clock.",
  "wrapper-classic": "Given an inner word, enter one word that wraps around it (2 letters each side) to advance to the next. You have 2 minutes.",
  "wrapper-survival": "Given an inner word, find one valid wrapper in 8 seconds. Each success brings a new word.",
  "crack-classic": "10 rounds: given two boundary letters on each side, type a word that fits in the middle to form a valid deep shell word.",
  "crack-survival": "Given boundary pairs, crack the shell before the 8 second timer runs out. Each success resets the clock.",
};
...
export function DeepShellWordsGame({
  groupSeed,
  locked,
  initialMode,
  isUntimed,
  ...
  const fetchWrapperPuzzle = useCallback(async (seed: number) => {
    const res = await fetch(`/api/games/deep-shell-words/puzzle?seed=${seed}`, { credentials: "include" });
    if (!res.ok) throw new Error("Failed to fetch puzzle");
    return (await res.json()) as { middle: string; count: number };
  }, []);

  const fetchCrackPair = useCallback(async (seed: number) => {
    const res = await fetch(`/api/games/deep-shell-words/crack?seed=${seed}`, { credentials: "include" });
    ...
    try {
      if (variation === "crack") {
        ...
        const res = await fetch(
          `/api/games/deep-shell-words/validate?word=${encodeURIComponent(outer)}`,
          { credentials: "include" }
        );
        ...
        if (!data.valid) {
          setFeedback({ type: "err", message: `"${outer}" is not a valid deep shell word` });
          clearFeedback();
          setInput("");
          return;
        }
      ...
      const res = await fetch(
        `/api/games/deep-shell-words/validate?word=${encodeURIComponent(word)}`,
        { credentials: "include" }
      );
      if (!res.ok) throw new Error("Validation request failed");
      const data = (await res.json()) as { valid: boolean; innerWord: string | null };
      ...
      if (!data.valid) {
        setFeedback({ type: "err", message: `"${word}" is not a valid deep shell word` });
        setInput("");
        clearFeedback();
        return;
      }
        ...
        <CardContent className="p-6 space-y-5">
          ...
          {gameStatus === "playing" && (
            <div className="space-y-4">
              ...
              {variation === "wrapper" && puzzleMiddle && (
                <div className="text-center p-4 rounded-lg bg-muted/50">
                  ...
                  <div className="text-xs text-muted-foreground mt-1">
                    e.g. __{puzzleMiddle}__ (add two letters on each end)
                  </div>
              ...
              {/* Crack: boundary pair display with input in middle */}
              {variation === "crack" && crackPair && (
                <div className="text-center p-4 rounded-lg bg-muted/50 space-y-3">
                  <div className="text-xs text-muted-foreground">Type a word that fits between these letter pairs:</div>
                  <div className="flex items-center justify-center gap-2">
                    <div
                      className="flex h-12 min-w-[3.5rem] px-3 items-center justify-center rounded-lg bg-primary text-primary-foreground text-xl font-bold font-mono tracking-widest"
              ...
              {/* Found words list (not shown in crack) */}
              {variation !== "crack" && (
                      ...
                      {foundWords.map((fw, i) => (
                        ...
                        >
                          <div className="font-mono text-sm">
                            <span className="text-muted-foreground">{fw.outer.slice(0, 2)}</span>
                            <span className="text-foreground font-bold">{fw.inner}</span>
                            <span className="text-muted-foreground">{fw.outer.slice(-2)}</span>
