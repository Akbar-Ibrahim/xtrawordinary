...
**Word Wars (Solo Bracket Tournament):** Signed-in users register for timed bracket tournaments. On deadline, the server draws an ELO-seeded single-elimination bracket padded to next power-of-2 with byes. Each match is a best-of-3 series across random duel game slugs. DB tables: `word_wars_tournaments`, `word_wars_registrations`, `word_wars_matches`, `word_wars_match_games`, `word_wars_champions`. `finalizeGame` in `duel-ws.ts` calls `resolveWordWarsGame` (in `server/word-wars-engine.ts`) which looks up the match game by room code, records the winner, checks if the series (first to 2 wins) is decided, advances the bracket, and creates next-round matches. Tournament completes when the final match is decided; the winner receives a `word_war_champion` notification. REST: `POST /api/word-wars`, `GET /api/word-wars`, `GET /api/word-wars/champions` (global Hall of Fame), `GET /api/word-wars/:id` (embeds `players` map), `POST /api/word-wars/:id/register`, `POST /api/word-wars/:id/draw`, `GET /api/word-wars/matches/:matchId`, `POST /api/word-wars/matches/:matchId/games/:gameNumber/start`, `GET /api/users/:id/championships`. Notification types: `word_war_matched`, `word_war_round_start`, `word_war_champion` (with icons/colors in nav). Frontend: lobby `/word-wars`, bracket `/word-wars/:id`, "Wars" nav link, Word Wars admin tab, champion badge on profile.
