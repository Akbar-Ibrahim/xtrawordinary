...
The real-time 1-on-1 Duel system for Word Chain is fully implemented. Key components: WebSocket server at `/ws/duel`, typed message protocol (`shared/duel-protocol.ts`), `DuelRoomRegistry` managing room lifecycle (30-second forfeit timers, stale-room sweep every 30 min), three database tables (`duel_challenges`, `duel_sessions`, `duel_ratings`), full REST API for challenge lifecycle, and ELO ratings. The duel UI lives at `/duel/:roomCode` with a phase machine (waiting → countdown → playing → over). Turn engine (`DuelTurnEngine` + `WordChainDuelAdapter`) enforces 8-second turns with server-authoritative timeouts and lives. Reconnect snapshots (`room:state`) support mid-game rejoin. **Known limitation:** in-progress duels are not resumable after a server process restart — only waiting-room/accepted challenge metadata (seed, startWord) is persisted. Deployment requires a DB migration to add `seed` and `start_word` columns to the `duel_challenges` table.
