...
**Quiz Master** (core infrastructure, Task #106): Signed-in users can create shareable quiz sessions for 9 supported games. A quiz session has a title, 8-character share code, and optional close date. Anyone with the link can play and submit their score. Scores are collected in a ranked leaderboard visible on the play page; the creator gets a private Results Dashboard. DB tables: `quiz_sessions` (creatorId, gameSlug, title, shareCode, params JSON, closesAt), `quiz_session_scores` (sessionId, userId, score — unique per user per session). APIs: `POST /api/quiz-sessions` (auth required), `GET /api/quiz-sessions/my` (auth required), `GET /api/quiz-sessions/:code` (public), `POST /api/quiz-sessions/:code/scores` (auth required, 409 if already submitted), `GET /api/quiz-sessions/:code/scores` (public). Pages: `/quiz/:code` (play lobby + game + live scores), `/quiz/:code/results` (creator-only dashboard). Create Quiz button on game-detail pages for supported games (QUIZ_MASTER_GAME_SLUGS). Seed derived from session.id for deterministic game variant. Supported games: letter-hunt, letter-frequency, letter-position, letter-balance, letter-pool, word-length, definition-match, word-roots, progressive-reveal.
...
A global leaderboard allows players to compare scores across different games. User profiles display public information, stats, and achievements. A friends system enables users to connect, send requests, and manage their friend lists. Friend challenges allow users to challenge friends in specific games with a fully automated flow: the challenger picks a game + friend, plays a seeded game, and their score is automatically submitted. The receiver plays the exact same puzzle (same seed) and their score is auto-submitted on completion. The challenger sees a "NEW" notification badge when the challenge is resolved. The `friend_challenges` table has `seed` (int nullable) and `senderViewed` (boolean) columns. New API endpoints: `GET /api/challenges/:id` (fetch single challenge), `POST /api/challenges/:id/viewed` (mark sender as having seen the result). Challenges are restricted to the 18 games that support `groupSeed` prop. Challenge entry point: game-detail page "Challenge a Friend" button, or Friends page "Challenge" button next to each friend.
